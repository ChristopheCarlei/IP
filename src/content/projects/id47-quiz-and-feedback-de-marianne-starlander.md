---
id: 47
title: "Quiz & feedback de Marianne Starlander"
author: ""
photo: /images/projet/Susan_Pickford_Marianne_Starlander.jpg
faculty: ""
course: ""
level: ""
students: ""
year: ""
category: ""
innovation_type: ""
observation_accepted: false
description_short: "L’enseignante propose des quiz formatifs et sommatifs accessibles depuis l’espace Moodle du cours. Un des travaux pratiques inclut une évaluation par les pairs pour laquelle les étudiant-es utilisent une grille d'évaluation pour noter et commenter les travaux de leurs camarades."
featured: false
status: actif
---

## Situation de départ

Ces dernières années, l’enseignante a vu le nombre d’étudiant-es grandement augmenter pour ce cours obligatoire. Il lui a fallu repenser l’organisation du cours afin de garder tout autant d’activités pour les étudiant-es en limitant le temps nécessaire à la correction de celles-ci. Elle a mis en place des quiz corrigés automatiquement sur Moodle et des travaux pratiques (TP) que les étudiant-es évaluent entre eux/elles.

## Mise en place

L’enseignante propose des quiz formatifs et sommatifs accessibles depuis l’espace Moodle du cours.
Les premiers, non notés, permettent aux étudiant-es de vérifier leur compréhension du cours. Ils sont composés de questions de type vrai-faux, appariement (une liste de possibilités à lier) et choix multiples.
Les seconds, notés cette fois, accompagnent les travaux pratiques (TP). Les étudiant-es doivent effectuer des TP après lesquels ils/elles ont l’occasion de relire le cours si besoin avant de remplir un quiz. Ces quiz comportent des questions directement liées aux TP ainsi que des questions plus théoriques. Le rendu du TP est une condition sine qua non afin de pouvoir remplir le quiz et que celui-ci soit noté. Seules les réponses aux quiz comptent pour la note de l’étudiant-e aux TP.
Les derniers sont ceux que propose l’enseignante pour l’évaluation finale du cours. Ils sont composés de 2 à 3 questions ouvertes et de QCM à une ou plusieurs réponses possibles.
Un des TP proposés s’accompagne d’une évaluation par les pairs. Chaque étudiant-e effectue 3 travaux de recherche documentaire qu’il/elle dépose ensuite sur Moodle. L’étudiant-e doit ensuite évaluer et commenter 3 des travaux remis par ses camarades. Pour effectuer cette évaluation, il/elle dispose d’une grille d’évaluation qui répertorie les points d’intérêts auxquels prêter attention lors de la correction. La moyenne des notes reçues à la suite de l’évaluation par les pairs constitue la note de l’étudiant-e à ce TP.
Retour et conseils sur la mise en place d’un tel projet
Le grand intérêt des quiz sur Moodle tient dans le fait que la correction de ceux-ci soit automatique. Cela permet un gain de temps pour l’enseignant-e (qui doit tout de même vérifier la globalité des corrections), mais également pour les étudiant-es qui disposent d’un feedback direct sur leur performance au quiz.
Des ateliers proposés par le Pôle SEA permettent de voir comment des enseignant-es de l’UNIGE utilisent des quiz sur Moodle, et donnent des conseils sur la formulation des questions et des options de réponse. L’implémentation d’un quiz pour l’examen nécessite par ailleurs de s’adresser au service dédié de l’université afin qu’il le valide et applique les paramètres nécessaires.
L’utilisation de l’évaluation par les pairs permet également pour les étudiant-es d’avoir un retour rapide, mais aussi plus riche puisque 3 pairs évaluent et commentent le travail de chaque étudiant-e. Ce format incite les étudiant-es à s’investir davantage dans l’apprentissage, car ils/elles doivent prendre du recul sur ce qui a été fourni comme travail par les autres. Une réponse différente de la leur n’étant pas forcément fausse, cela leur ouvre le champ de leurs connaissances.

## Avis des étudiant·es

> « Les TP et les quiz permettent de mieux assimiler la matière. »
> « Les thèmes abordés et les exercices à faire à domicile (quiz, TP) permettent de se familiariser avec la recherche documentaire, notamment sur internet. »

